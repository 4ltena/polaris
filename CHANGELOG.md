...
- マウスホイールでの会話履歴スクロール(v0.6.0で一度追加)を廃止した。端末のマウスキャプチャを有効化している間は、端末ネイティブのドラッグ選択・コピーが一切効かなくなる不具合と表裏一体だったため。純正codex(`codex-rs`)にはマウス処理コードが一切存在せず、常にネイティブ選択を優先する設計だったことを実コードで確認して同じ方針にした。スクロールは引き続きPageUp/PageDownキーで行う
