...
- `read`等のツール結果プレビュー行に、インラインコード/フェンス付きコードブロックと同じ暗灰色背景を適用し、コード部分だと視覚的に分かるようにした(全言語対応のシンタックスハイライトはスコープ外)
...
- 入力欄の灰色の網掛けが、実際に入力した文字幅を超えて行全体(空白部分も含む)に広がっていた不具合。プロンプト・テキストの幅だけに網掛けを限定した
