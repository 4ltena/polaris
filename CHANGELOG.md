...
## [0.8.0] — 2026-08-27 "Antares"

機能を2つ足した。Codexプロバイダのreasoning item継続と、自動履歴圧縮である。前者はResponses APIが返す暗号化済みのreasoning itemを会話履歴に保持し、次のターンでそのまま送り返す。後者は会話が閾値を超えた時点で、古いターンを1件の要約メッセージへ置き換える。閾値は開発中に100,000から200,000へ引き上げたが、その判断の根拠は実測していない。同種のキャッシュ効率の主張がv0.7.0で再現しなかった経緯があるため、コード側のコメントにも未検証である旨を残した。
