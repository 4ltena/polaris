...
- 入力履歴の呼び出し: Ctrl+P/Ctrl+N(readline/Emacsの`previous-history`/`next-history`と同じキー)で、今回のTUIセッション内で送信した過去の入力を辿れるようにした(セッションを跨いだ永続化はしない)
- 会話履歴の自前スクロール: `history: Vec<HistoryLine>` にヘッダー・会話・ツール進捗を蓄積し、`scroll_offset` に応じた窓だけを毎フレーム描画する方式へ変更。Up/Downキーで履歴をスクロールできる(候補ポップアップ表示中はポップアップの選択に、それ以外は履歴スクロールに使う)
...
- ターン実行中もUp/Downキーで履歴をスクロールできるようにした(従来はアイドル時のみ)
