...
polaris は `lookup`（skill 検索）ツールを呼ばない限り、830 件設置後も常時コンテキストは 3 ターンでそれぞれ 546、599、746 トークンと、skill を 1 件も置かない場合と変わらない。これは `budget.rs` の不変条件テストが保証する性質を実運用でなぞったものである。`lookup` を実際に呼ばせた場合（同じ 830 件のコーパスに対する検索を明示的に指示したタスクで別途測定）は、その呼び出し 1 回につき約 1,200〜2,400 トークンが該当ターンにだけ加わる——codex・pi のように毎ターン、使うかどうかに関係なく払い続けるコストとは性質が異なり、検索した回だけ・検索した分だけ払う。
