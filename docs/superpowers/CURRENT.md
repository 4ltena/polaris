...
| | |
| --- | --- |
| ブランチ | `feat/m1-headless-loop`（HEAD `dbb5e82`） |
| 進行中の計画 | なし |
| 直近で終えた計画 | `docs/superpowers/plans/2026-08-18-polaris-m25-codex-provider.md` |
| 仕様 | `docs/superpowers/specs/2026-08-16-polaris-harness-design.md`、`docs/superpowers/specs/2026-08-18-polaris-codex-provider-design.md` |
| 版の方針 | v1.0.0 = `hamal`。M1・M2・M2.5 が完了し、タグ付けの条件が揃った。タグ付け自体は利用者の承認を待つ準備段階のまま |
...
裁定の全文は各計画の台帳の `Ruling:` 行にある。M2 の台帳には 20 件、M2.5 の台帳にはこの文書へ移した最終レビューの裁定を含めて記録している。
...
ブランチを締める。M2.5 の作業領域を消し、`superpowers:finishing-a-development-branch`
