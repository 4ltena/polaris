...
| 版の方針 | v1.0.0 = `hamal`。M1 と M2 は完了済みだが、タグ付けは M2.5 の完了を待つ |
...
| M2.5 | Codex プロバイダ（ChatGPT サブスク OAuth） | 仕様済み・未着手 |
| M3a | skills ローダと skill ツール | 完了 |
| M3b | BM25 ルータと評価コーパス | 未着手 |
| M4 | subagent、波、継続波 | 未着手 |
| M5 | TUI、圧縮、マルチプロバイダ、セッション永続化 | 未着手 |

M1 と M2 が揃ったので、内容としては v1.0.0 (`hamal`) の水準に達している。ただしタグ付けは M2.5 の完了を待つ。一度も実モデルへ繋いだことのない状態で 1.0.0 を打つと看板と中身がずれるためで、利用者の判断による。M3 以降は 1.x として積む。M3a を M2 より先に進めたのも利用者の指示による。
...
M2.5 の実装計画を書き、Codex プロバイダを作る。仕様は
`docs/superpowers/specs/2026-08-18-polaris-codex-provider-design.md`。

そのうえで v1.0.0 のタグ付けの判断へ進み、M3b（BM25 ルータと評価コーパス）へ移る。
ブランチを締める操作（push、マージ）は利用者の承認を待つ。
